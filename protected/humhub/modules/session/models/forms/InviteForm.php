...
class InviteForm extends Model
{
    ...
    {
        return array(
            'invite' => Yii::t('SessionModule.forms_SessionInviteForm', 'Invites'),
    ...
    public function checkInvite($attribute, $params)
    {
        ...
        if ($this->$attribute != "") {
            ...
            foreach ($invites as $userGuid) {
                ...
                if ($user != null) {
                    $membership = SessionMembership::findOne(['session_id' => $this->session->id, 'user_id' => $user->id]);
                    
                    if ($membership != null && $membership->status == SessionMembership::STATUS_MEMBER) {
                        $this->addError($attribute, Yii::t('SessionModule.forms_SessionInviteForm', "User '{username}' is already a member of this session!", ['username' => $user->getDisplayName()]));
                        continue;
                    } else if($membership != null && $membership->status == SessionMembership::STATUS_APPLICANT) {
                        $this->addError($attribute, Yii::t('SessionModule.forms_SessionInviteForm', "User '{username}' is already an applicant of this session!", ['username' => $user->getDisplayName()]));
                        continue;
                    }
                    $this->invites[] = $user;
                } else {
                    $this->addError($attribute, Yii::t('SessionModule.forms_SessionInviteForm', "User not found!"));
                    continue;
                }
            }
        }
