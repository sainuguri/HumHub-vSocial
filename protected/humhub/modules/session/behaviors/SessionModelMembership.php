 ...
 */
class SessionModelMembership extends Behavior
{

    private $_spaceOwner = null;

    /**
     * Checks if given Userid is Member of this Session.
     *
     * @param type $userId
     * @return type
     */
    ...
    public function getMembership($userId = null)
    {
        ...
        return SessionMembership::findOne(['user_id' => $userId, 'session_id' => $this->owner->id]);
    }
    ...
    public function inviteMember($userId, $originatorId)
    {
        ...
        if ($membership != null) {
            switch ($membership->status) {
                case SessionMembership::STATUS_APPLICANT:
    ...
    {
        $notification = new \humhub\modules\session\notifications\Invite([
            'source' => $this->owner,
